...
class GraphSVX:
    ...
    def explain(
        ...
        for node_index in node_indexes:
            ...
            if self.gpu:
                ...
                with torch.no_grad():
                    true_conf, true_pred = (
                        self.model(self.data.x.cuda(), self.data.edge_index.cuda())
                        .exp()[node_index]
                        .max(dim=0)
                    )
            else:
            ...
            self.neighbours, _, _, edge_mask = torch_geometric.utils.k_hop_subgraph(
                node_idx=node_index, num_hops=hops, edge_index=self.data.edge_index
            )

            # Retrieve 1-hop neighbours of v
            one_hop_neighbours, _, _, _ = torch_geometric.utils.k_hop_subgraph(
                node_idx=node_index, num_hops=1, edge_index=self.data.edge_index
            )
            ...
            if args_hv == "compute_pred_subgraph":
                feat_idx, discarded_feat_idx = self.feature_selection_subgraph(
                    node_index, args_feat
                )
                ...
                feat_idx, discarded_feat_idx = self.feature_selection(
                    node_index, args_feat
                )
            ...
            self.neighbours = list(
                range(
                    int(
                        ...
                        - np.sum(np.diag(self.data.edge_index[graph_index]))
                    )
                )
            )
    ...
    def graph_classification(
        self, graph_index, num_samples, D, z_, args_K, args_feat, true_pred
    ):
        """Construct z' from z and compute prediction f(z') for each sample z
            In fact, we build the dataset (z, f(z')), required to train the weighted linear model.
        ...
        for key, ex_nei in tqdm(excluded_nei.items()):
            ...
            if self.gpu:
                with torch.no_grad():
                    proba = self.model(
                        X.unsqueeze(0).cuda(), A.unsqueeze(0).cuda()
                    ).exp()
            else:
                with torch.no_grad():
...
class Greedy:
    ...
    def explain(
        self, node_index=0, hops=2, num_samples=0, info=False, multiclass=False, *unused
    ):
        """
        ...
        if self.gpu:
            ...
            with torch.no_grad():
                probas = self.model(
                    x=self.data.x.cuda(), edge_index=self.data.edge_index.cuda()
                ).exp()[node_index]
        else:
            with torch.no_grad():
                probas = self.model(
                    x=self.data.x, edge_index=self.data.edge_index
                ).exp()[node_index]
        ...
        if multiclass:
            ...
            for i, idx in enumerate(feat_idx):
                ...
                if self.gpu:
                    with torch.no_grad():
                        probas_ = self.model(
                            x=x_.cuda(), edge_index=self.data.edge_index.cuda()
                        ).exp()[
                            node_index
                        ]  # [label].item()
                else:
                    with torch.no_grad():
                        probas_ = self.model(
                            x=x_, edge_index=self.data.edge_index
                        ).exp()[
                            node_index
                        ]  # [label].item()
    ...
    def explain_nei(
        self, node_index=0, hops=2, num_samples=0, info=False, multiclass=False
    ):
        """Greedy explainer - only considers node features for explanations"""
        # Construct k hop subgraph of node of interest (denoted v)
        neighbours, _, _, edge_mask = torch_geometric.utils.k_hop_subgraph(
            node_idx=node_index, num_hops=hops, edge_index=self.data.edge_index
        )
        ...
        if multiclass:
            ...
            for i, nei_idx in enumerate(self.neighbours):
                ...
                if self.gpu:
                    with torch.no_grad():
                        probas_ = self.model(
                            x=self.data.x.cuda(), edge_index=A_.cuda()
                        ).exp()[node_index]
                else:
                    with torch.no_grad():
    ...
    def explain(
        ...
        **kwargs
        ...
        if multiclass:
            ...
            solver = LassoLars(
                self.rho, fit_intercept=False, normalize=False, positive=True
            )
...
class LIME:
    ...
    def explain(
        self, node_index, hops, num_samples, info=False, multiclass=False, **kwargs
    ):
...
class SHAP:
    ...
    def explain(
        self, node_index=0, hops=2, num_samples=10, info=True, multiclass=False, *unused
    ):
        """
        ...
        with torch.no_grad():
            if self.gpu:
                ...
                true_conf, true_pred = (
                    self.model(x=self.data.x, edge_index=self.data.edge_index)
                    .exp()[node_index][0]
                    .max(dim=0)
                )
        ...
        fz = self.compute_pred(node_index, num_samples, z_, multiclass, true_pred)
    ...
    def compute_pred(self, node_index, num_samples, z_, multiclass, true_pred):
        ...
        for i in range(num_samples):
            ...
            with torch.no_grad():
                if self.gpu:
                    proba = self.model(
                        x=X.cuda(), edge_index=self.data.edge_index.cuda()
                    ).exp()[node_index]
                else:
...
class GNNExplainer:
    ...
    def explain(
        self, node_index, hops, num_samples, info=False, multiclass=False, *unused
    ):
        num_samples = num_samples // 2
        ...
        if multiclass:
            ...
            # Attribute an importance measure to each node = sum of incident edges' importance
            self.coefs = torch.zeros(self.neighbours.shape[0])
            for i, val in enumerate(dico.values()):
                self.coefs[i] = sum(val)

...
from src.data import (
    add_noise_features,
    add_noise_neighbours,
    extract_test_nodes,
    prepare_data,
)
from src.explainers import LIME, SHAP, GNNExplainer, GraphLIME, GraphSVX, Greedy, Random
...
def filter_useless_features_multiclass(
    args_dataset,
    args_model,
    args_explainers,
    args_hops,
    ...
    if args_model == "GCN":
        model = GCN(
            input_dim=data.x.size(1), output_dim=data.num_classes, **eval(hyperparam)
        )
    else:
        model = GAT(
            input_dim=data.x.size(1), output_dim=data.num_classes, **eval(hyperparam)
    ...
    for c, explainer_name in enumerate(args_explainers):
        ...
        for node_idx in tqdm(node_indices, desc="explain node", leave=False):
            ...
            num_noise_feats = []
            true_conf, predicted_class = (
                model(x=data.x, edge_index=data.edge_index).exp()[node_idx].max(dim=0)
            )

            for i in range(data.num_classes):
                # Store indexes of K most important node features, for each class
                feat_indices = (
                    np.abs(coefs[: explainer.F, i]).argsort()[-args_K:].tolist()
                )
        ...
        if info:
            ...
            print(
                "There are {} noise features found in the explanations of {} test samples, an average of {} per sample".format(
                    sum(total_num_noise_feats),
                    args_test_samples,
                    sum(total_num_noise_feats) / args_test_samples,
                )
            )
            ...
            print(
                "Proportion of non-zero noisy features among non-zero features: {:.2f}%".format(
                    perc
                )
            )
            ...
            print(
                "Proportion of explanations showing noisy features: {:.2f}%".format(
                    perc
                )
            )
            ...
            if sum(total_num_noise_feat_considered) != 0:
                ...
                perc2 = (
                    ...
                    * (
                    ...
                    / (
                        data.num_classes
                        * (sum(F) - sum(total_num_noise_feat_considered))
                    )
                )
                print(
                    "Proportion of noisy features found in explanations vs normal features (among considered ones): {:.2f}% vs {:.2f}%, over considered features only".format(
                        perc, perc2
                    )
                )
...
def filter_useless_nodes_multiclass(
    args_dataset,
    args_model,
    args_explainers,
    args_hops,
    ...
    for c, explainer_name in enumerate(args_explainers):
        ...
        for node_idx in tqdm(node_indices, desc="explain node", leave=False):
            ...
            num_noisy_nodes = len(
                [
                    n_idx
                    for n_idx in explainer.neighbours
            ...
            for i in range(data.num_classes):
                ...
                # Number of noisy features that appear in explanations - use index to spot them
                num_noise_nei = sum(
                    idx >= (explainer.neighbours.shape[0] - num_noisy_nodes)
        ...
        if info:
            ...
            print(
                "There are {} noise neighbours found in the explanations of {} test samples, an average of {} per sample".format(
                    sum(total_num_noise_neis),
                    args_test_samples,
                    sum(total_num_noise_neis) / args_test_samples,
                )
            )
            ...
            print(
                "Proportion of explanations showing noisy neighbours: {:.2f}%".format(
                    100
                    * sum(total_num_noise_neis)
                    / (args_K * args_test_samples * data.num_classes)
                )
            ...
            perc2 = (
                ...
                * (
                ...
                / (np.sum(M) - sum(total_num_noisy_nei))
            )
            print(
                "Proportion of noisy neighbours found in explanations vs normal features: {:.2f}% vs {:.2f}".format(
                    perc, perc2
                )
            )

            print(
                "Proportion of nodes in subgraph that are noisy: {:.2f}%".format(
                    100 * sum(total_num_noisy_nei) / sum(total_neigbours)
                )
            )

            print(
                "Proportion of noisy neighbours in subgraph found in explanations: {:.2f}%".format(
                    100
                    * sum(total_num_noise_neis)
                    / (sum(total_num_noisy_nei) * data.num_classes)
                )
            )
        ...
        # Do for all benchmarks (with diff colors) and plt.show() to get on the same graph
        total_num_noise_neis = [
            item / data.num_classes for item in total_num_noise_neis
        ]
...
def study_attention_weights(data, model, args_test_samples):
    ...
    edges, alpha1 = alpha[0][:, : -(data.x.size(0))], alpha[1][: -(data.x.size(0)), :]
